...
def majority_intent(examples):
    labels = [
        e["annotation"]["intent"]
        for e in examples
        if e.get("partition") == "development"
        and e.get("annotation")
        and e["annotation"].get("eligible", True)
    ]
...
def predict(
    ...
    if evidence_mode == "removed":
        ...
        evidence = [
            {**e, "score": 0.0}
            for e in retriever.corpus[-3:]
            if e["group_id"] != example.get("group_id")
        ]
    ...
    try:
        if system == "trivial":
            ...
            decision = {
                "decision": "escalate",
                "reason_codes": ["baseline_always_escalates"],
                "reason": "Trivial baseline always requests human review.",
            }
        elif system == "simple":
            if evidence:
                ...
                support = True
                explanation = (
                    "Nearest observed historical reply; not a verified resolution."
                )
            ...
            if provider is None:
                raise ValueError(
                    "Live agent requires a configured model provider; no simulated scores are substituted."
                )
            meta = provider.complete(
                AGENT_PROMPT,
                {
                    "incoming": incoming,
                    "intents": INTENTS,
                    "evidence": [
                        {
                            k: e[k]
                            for k in ["id", "text", "historical_reply", "evidence_type"]
                        }
                        for e in evidence
                    ],
                },
            ...
            if (
                ...
                or not all(isinstance(i, str) for i in used)
                or not set(used) <= {e["id"] for e in evidence}
            ):
        ...
        decision = {
            ...
            "reason": "Processing failed; human review required.",
        }
    return {
        "id": example["id"],
        "system": system,
        "partition": example.get("partition"),
        "intent": intent,
        "draft_reply": reply,
        **decision,
        "evidence_ids": used,
        "retrieved": [{"id": e["id"], "score": e["score"]} for e in evidence],
        "support_explanation": explanation,
        "support_established": support,
        "latency_seconds": round(time.perf_counter() - start, 6),
        "provider": {k: v for k, v in meta.items() if k != "output"},
        "error": error,
        "evidence_mode": evidence_mode,
        "threshold": threshold,
        "prompt_hash": fingerprint(AGENT_PROMPT) if system == "agent" else None,
    }
