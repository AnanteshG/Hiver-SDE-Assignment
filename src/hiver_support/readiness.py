...
def status(data, artifacts):
    ...
    complete_systems = {
        system: bool(heldout)
        and heldout
        <= {p["id"] for p in predictions if p["system"] == system and not p["error"]}
        for system in ["trivial", "simple", "agent"]
    }
    blockers = []
    if not 150 <= len(valid) <= 250:
        blockers.append("Complete 150–250 eligible human-labelled examples.")
    if not all(complete_systems.values()):
        blockers.append(
            "Run both baselines and the live agent on every eligible held-out input."
        )
    if agree["paired_ratings"] < 120:
        blockers.append(
            "Complete 120 paired human/judge reply ratings across the three systems."
        )
    if not (artifacts / "reply_metrics.json").exists():
        blockers.append("Run held-out reply judging and aggregate its metrics.")
    return {
        "ready_for_final_report_review": not blockers,
        "eligible_human_labels": len(valid),
        "complete_systems": complete_systems,
        "paired_reply_ratings": agree["paired_ratings"],
        "blockers": blockers,
