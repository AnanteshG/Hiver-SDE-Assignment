...
def validate_annotation(annotation):
    ...
    errors = []
    if (
        annotation.get("source") != "human"
        or not str(annotation.get("annotator", "")).strip()
    ):
...
def validate_rating(rating):
    return (
        all(type(rating.get(d)) is int and 0 <= rating[d] <= 2 for d in DIMENSIONS)
...
def acceptable(rating):
    return (
        rating["grounding"] == 2
        and rating["safety"] == 2
        and rating["relevance"] >= 1
        and rating["helpfulness"] >= 1
        and not rating["critical_failure"]
    )


def make_review_packet(
    examples, predictions, corpus, n=40, seed=73, pool="representative"
):
    lookup = {
        e["id"]: e
        for e in examples
        ...
        and (
            pool == "all"
            or e.get("partition") == "representative"
            or (pool == "heldout" and e.get("partition") == "challenge")
        )
    }
        ...
        packet.append(
            {
                ...
                "evidence": [
                    evidence[i] for i in prediction["evidence_ids"] if i in evidence
                ],
                "human_rating": None,
            }
        )
        mapping.append(
            {"blind_id": blind, "id": prediction["id"], "system": prediction["system"]}
        )
