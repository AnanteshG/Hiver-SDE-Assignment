    ...
    for system in sorted({p["system"] for p in predictions}):
        ...
        for partition in ["development", "representative", "challenge"]:
            ...
            selected = [
                ...
                if p["system"] == system
                and p["id"] in lookup
                and lookup[p["id"]]["partition"] == partition
            ]
            ...
            labelled = [
                p
                for p in selected
                if not validate_annotation(lookup[p["id"]].get("annotation"))
                and lookup[p["id"]]["annotation"]["eligible"]
            ]
            correct = sum(
                p["intent"] == lookup[p["id"]]["annotation"]["intent"] for p in labelled
            )
            confusion = Counter(
                (lookup[p["id"]]["annotation"]["intent"], p["intent"]) for p in labelled
            )
            per_intent = {}
            f1s = []
            for intent in INTENTS:
                ...
                predicted = sum(
                    v for (_, pred), v in confusion.items() if pred == intent
                )
            ...
            required = [
                p for p in labelled if lookup[p["id"]]["annotation"]["human_required"]
            ]
            ...
            results[system][partition] = {
                ...
                "labelled_evaluated": len(labelled),
                "missing_predictions": len(expected) - len(selected),
                "intent_accuracy": fraction(correct, len(labelled)),
                "intent_macro_f1": sum(f1s) / len(f1s) if labelled else None,
                "per_intent": per_intent,
                "confusion": [
                    {"gold": g, "predicted": p, "count": v}
                    for (g, p), v in sorted(confusion.items())
                ],
                "auto_coverage": fraction(len(auto), len(labelled)),
                "unsafe_auto_rate": fraction(unsafe, len(auto)),
                "unsafe_auto_wilson_95": wilson(unsafe, len(auto)),
                "escalation_recall": fraction(
                    sum(p["decision"] == "escalate" for p in required), len(required)
                ),
                "processing_error_rate": fraction(
                    sum(p["error"] is not None for p in selected), len(selected)
                ),
                "unlabelled_auto_count": sum(
                    p["decision"] == "auto_handle" for p in selected
                ),
                ...
                "total_tokens": sum(
                    p.get("provider", {}).get("usage", {}).get("total_tokens", 0)
                    for p in selected
                ),
                "cost_usd": None,
            }
...
def weighted_kappa(human, judge):
    ...
    expected = sum(
        a[i] * b[j] * ((i - j) / 2) ** 2 for i in range(3) for j in range(3)
    ) / (n * n)
...
def agreement(packet, judged):
    ...
    pairs = [
        (r["human_rating"], judges[r["blind_id"]]["rating"])
        for r in packet
        if r.get("human_rating")
        and r["human_rating"].get("source") == "human"
        and str(r["human_rating"].get("annotator", "")).strip()
        and r["blind_id"] in judges
        and validate_rating(r["human_rating"])
        and validate_rating(judges[r["blind_id"]]["rating"])
    ]
    output = {
        "paired_ratings": len(pairs),
        "status": "measured" if pairs else "pending_human_ratings",
        "dimensions": {},
    }
    for dimension in DIMENSIONS:
        ...
        output["dimensions"][dimension] = {
            "exact_agreement": fraction(
                sum(a == b for a, b in zip(human, judge)), len(pairs)
            ),
            "weighted_kappa": weighted_kappa(human, judge),
        }
    confusion = Counter((acceptable(h), acceptable(j)) for h, j in pairs)
    output["acceptability_confusion"] = {
        f"human_{h}_judge_{j}": confusion[h, j]
        for h in (False, True)
        for j in (False, True)
    }
    output["false_approval_rate_among_human_rejected"] = fraction(
        confusion[False, True], sum(not acceptable(h) for h, j in pairs)
    )
    return output
    ...
    for system in sorted({r["system"] for r in mapping}):
        ...
        for partition in ["development", "representative", "challenge"]:
            selected = [
                r
                for r in mapping
                if r["system"] == system and lookup[r["id"]]["partition"] == partition
            ]
            valid = [
                scores[r["blind_id"]]["rating"]
                for r in selected
                if r["blind_id"] in scores
                and not scores[r["blind_id"]].get("error")
                and validate_rating(scores[r["blind_id"]]["rating"])
            ]
            output[system][partition] = {
                "requested": len(selected),
                "valid_ratings": len(valid),
                "missing_or_failed": len(selected) - len(valid),
                "pass_rate": fraction(sum(acceptable(r) for r in valid), len(valid)),
                "dimension_means": {
                    d: statistics.mean(r[d] for r in valid) if valid else None
                    for d in DIMENSIONS
                },
            }
    return output
