...
class Provider:
    def __init__(self, model=None, cache_dir='.cache/llm', base_url=None):
        ...
        if not self.model or not self.base_url:
            raise ValueError('Set LLM_MODEL and LLM_BASE_URL for live calls. LLM_API_KEY is required by hosted providers.')
        if not self.base_url.startswith('https://') and not self.base_url.startswith(('http://localhost:', 'http://127.0.0.1:')):
            raise ValueError('Use HTTPS for hosted endpoints or localhost for a local model.')

    def complete(self, system, payload):
        request = {'model': self.model, 'temperature': 0, 'messages': [
            {'role': 'system', 'content': system}, {'role': 'user', 'content': json.dumps(payload, ensure_ascii=False)}],
            'response_format': {'type': 'json_object'}}
        cache = self.cache_dir / (fingerprint({'endpoint': self.base_url, 'request': request}) + '.json')
        if cache.exists():
        ...
        for attempt in range(3):
            try:
                req = urllib.request.Request(self.base_url + '/chat/completions', data=json.dumps(request).encode(), headers=headers)
        ...
        result = {'output': json.loads(content), 'usage': raw.get('usage', {}), 'model': raw.get('model', self.model),
                  'latency_seconds': time.perf_counter() - start, 'cache_hit': False}
