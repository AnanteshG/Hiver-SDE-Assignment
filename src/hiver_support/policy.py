...
def classify(text):
    counts = {
        intent: len(re.findall(r"\b(?:" + pattern + r")\b", text.lower()))
        for intent, pattern in PATTERNS.items()
    }
    return (
        max(counts, key=counts.get)
        if max(counts.values(), default=0)
        else "other_unclear"
    )
...
def gates(example, intent, evidence, reply, supported, threshold=0.3):
    ...
    if intent in {"account_access", "billing_purchase", "hardware_repair"} or re.search(
        r"\brefund|\bcharged\b|\bbilling\b|\bpassword\b|\bwarranty\b|\bcancel.{0,25}subscription",
        text,
    ):
        failures.append("account_or_specialist_required")
    if re.search(
        r"smoke|fire|burn(?:ing|t)|swoll|injur|stolen|hacked|fraud|suicid", text
    ):
    ...
    if re.search(
        r"(?:i|we)(?:'ve| have)? (?:checked|refunded|cancelled|canceled|updated your|reset your)|(?:send|share|provide).{0,45}(?:password|credit card|card number|verification code|email|phone number|serial number)",
        reply,
        re.I,
    ):
    ...
    if re.search(
        r"(?:updat\w*|upgrad\w*) (?:it |your device )?to (?:ios|version)\s*\d|(?:released|fixed).{0,40}(?:this week|today|yesterday)|latest (?:ios|version)",
        reply,
        re.I,
    ):
        failures.append("time_sensitive_advice")
    return {
        "decision": "escalate" if failures else "auto_handle",
        "reason_codes": failures or ["supported_low_risk_response"],
        "reason": "; ".join(code.replace("_", " ") for code in failures)
        if failures
        else "Supported next response; no account action or sensitive request identified.",
    }
