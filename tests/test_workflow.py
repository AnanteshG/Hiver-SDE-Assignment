...
class WorkflowTests(unittest.TestCase):
    ...
    def test_blinding_removes_labels_and_system(self):
        example = dict(
            ...
            text="help",
            context=[],
            annotation={"intent": "secret"},
            historical_reply="future",
        )
    ...
    def test_known_denominators_and_errors(self):
        annotation = dict(
            ...
            human_required=True,
            reason="test",
            response_requirements="test",
            prohibited_claims="test",
        )
        ...
        with self.assertRaises(ValueError):
            evaluate(examples, predictions * 2)

    def test_reply_metric_denominators(self):
        result = reply_summary(
            [dict(blind_id="a", id="x", system="simple")],
            [],
            [dict(id="x", partition="representative")],
        )
