...
class AgentTests(unittest.TestCase):
    def setUp(self):
        self.case = dict(
            id="one",
            group_id="g1",
            text="phone battery draining very fast",
            historical_reply="Which software version are you using?",
        ...
        self.example = dict(
            id="two",
            group_id="g2",
            text="my phone battery is draining very fast",
            context=[],
        )

    def test_self_group_excluded(self):
        self.assertEqual(
            self.retriever.search(self.case["text"], excluded_group="g1"), []
        )
    ...
    def test_wrong_intent_cannot_override_refund_gate(self):
        example = {
            **self.example,
            "text": "The battery is broken and I want a refund please.",
        }
        result = gates(
            example,
            "battery_power",
    ...
    def test_future_reply_not_sent_and_bad_citation_rejected(self):
        example = {
            **self.example,
            "historical_reply": "SECRET FUTURE",
            "annotation": {"intent": "battery_power"},
        }
    ...
    def test_missing_evidence_abstains(self):
        result = predict(
            self.example, "simple", self.retriever, evidence_mode="removed"
        )
        self.assertEqual(result["decision"], "escalate")
        ...
        class Fake:
            def complete(self, prompt, payload):
                return {
                    "output": dict(
                        intent="battery_power",
                        draft_reply="We have refunded your purchase.",
                        evidence_ids=["one"],
                        support_established=True,
                    )
                }
