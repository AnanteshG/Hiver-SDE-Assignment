...
st.set_page_config(page_title='Trust Review | Support Agent', page_icon='◈', layout='wide')
st.markdown('''<style>
...
if page=='Overview':
    ...
    if reviewed<len(examples):
        st.warning('Evaluation is pending human labels. Prediction counts are operational observations, not accuracy or safety evidence.')
    st.subheader('What this system proves—and what remains open')
    st.write('Each response has an intent, a next-step draft, and a handling decision. Inspect retrieved cases and explicit escalation reasons before judging whether an answer is trustworthy.')
    metrics_path=ARTIFACTS/'metrics.json'
    if metrics_path.exists():
        metrics=json.loads(metrics_path.read_text())
        partition=st.selectbox('Evaluation partition',['representative','challenge','development'])
        table=[]
    ...
    if not predictions:
        st.info('Run the pipeline to create predictions.'); st.stop()
    system=st.selectbox('System',sorted({p['system'] for p in predictions}))
    decision=st.selectbox('Handling',['all','auto_handle','escalate'])
    filtered=[p for p in predictions if p['system']==system and (decision=='all' or p['decision']==decision)]
    if not filtered:
        st.info('No matching cases.'); st.stop()
    ...
    st.caption('Read the annotation guide before starting. Predictions and future brand replies are hidden here to reduce anchoring.')
    if not examples: st.info('Prepare data first.'); st.stop()
    annotator=st.text_input('Your name or reviewer ID')
    partition=st.selectbox('Partition',['development','representative','challenge'])
    unfinished=st.checkbox('Only unlabelled examples',value=True)
    available=[e for e in examples if e['partition']==partition and (not unfinished or validate_annotation(e.get('annotation')))]
    if not available: st.success('This selection is complete.'); st.stop()
    selected=st.selectbox('Example',[e['id'] for e in available])
    example=next(e for e in available if e['id']==selected)
    for turn in example['context']: st.caption(f"{turn['role']}: {turn['text']}")
    st.write(example['text'])
    if example['context_incomplete']: st.warning('Some prior context is missing.')
    existing=example.get('annotation') or {}
    with st.form('annotation_'+selected):
        eligible=st.checkbox('Eligible English support message',value=existing.get('eligible',True))
        intent=st.selectbox('Primary intent',list(INTENTS),index=list(INTENTS).index(existing.get('intent','other_unclear')))
        st.caption(INTENTS[intent])
        handling=st.radio('Does the next response require human review?',['Choose a label','Yes','No'],index=0)
        reason=st.text_input('Handling reason',value=existing.get('reason',''))
        requirements=st.text_area('What must an acceptable reply accomplish?',value=existing.get('response_requirements',''))
        prohibited=st.text_area('What must the reply not claim or request?',value=existing.get('prohibited_claims',''))
        notes=st.text_area('Ambiguity or exclusion notes',value=existing.get('notes',''))
        confirmed=st.checkbox('I personally reviewed this example and chose these labels.')
        save=st.form_submit_button('Save human annotation',type='primary')
    if save:
        annotation=dict(source='human',annotator=annotator,eligible=eligible,intent=intent,
            human_required={'Yes':True,'No':False}.get(handling),reason=reason,response_requirements=requirements,
            prohibited_claims=prohibited,notes=notes,reviewed_at=datetime.now(timezone.utc).isoformat())
        errors=validate_annotation(annotation)
        if not confirmed: errors.append('Confirm personal review before saving')
        if errors: st.error('; '.join(errors))
        else:
            latest=read_jsonl(DATA/'candidates.jsonl')
            for e in latest:
                if e['id']==selected: e['annotation']=annotation
            write_jsonl(DATA/'candidates.jsonl',latest); st.rerun()
    ...
    if not pending: st.info('No unrated replies. Create a review packet after running the systems.'); st.stop()
    ...
    st.write('**Customer:**',row['text']); st.write('**Candidate reply:**',row['draft_reply'])
    with st.expander('Context and evidence',expanded=True):
        for turn in row['context']: st.write(turn['role']+': '+turn['text'])
        for e in row['evidence']: st.write(e['text']); st.caption(e['historical_reply'])
    st.caption('0 = fails · 1 = partial/minor concern · 2 = fully meets dimension. Consult docs/judge_rubric.md.')
    with st.form('rating_'+selected):
        ratings={d:st.selectbox(d.title(),[None,0,1,2],format_func=lambda v:'Choose a score' if v is None else str(v)) for d in DIMENSIONS}
        critical=st.checkbox('Critical failure: fabricated action, private credentials, or dangerous advice')
    ...
    if save:
        if not annotator.strip() or not confirmed or any(v is None for v in ratings.values()) or not rationale.strip():
            st.error('Complete all scores, reviewer ID, rationale, and personal-review confirmation.')
        else:
            latest=read_jsonl(path)
            for r in latest:
                if r['blind_id']==selected: r['human_rating']={**ratings,'critical_failure':critical,'rationale':rationale,'annotator':annotator,'source':'human'}
            write_jsonl(path,latest); st.rerun()

elif page=='Judge audit':
    st.subheader('Where the judge disagrees with a human')
    packet=read_optional(ARTIFACTS/'review_packet.jsonl'); judged=read_optional(ARTIFACTS/'judge_scores.jsonl')
    from hiver_support.metrics import agreement
    result=agreement(packet,judged)
    st.metric('Paired ratings',result['paired_ratings'])
    if not result['paired_ratings']: st.info('Human ratings and live judge outputs are required before agreement can be measured.')
    else:
        st.dataframe([{'Dimension':d,'Exact agreement':v['exact_agreement']['value'],'Weighted kappa':v['weighted_kappa']} for d,v in result['dimensions'].items()],hide_index=True)
        lookup={r['blind_id']:r for r in judged if not r['error']}
        for row in packet:
            human=row.get('human_rating'); judge=lookup.get(row['blind_id'],{}).get('rating')
            if human and judge and any(human[d]!=judge[d] for d in DIMENSIONS):
                with st.expander(row['blind_id']):
                    st.write(row['draft_reply']); st.json({'human':human,'judge':judge})

elif page=='Evidence stress test':
    st.subheader('Does the agent stop when its evidence disappears?')
    st.write('Compare normal, removed, and deliberately unrelated evidence. These controlled variants are separate from the real-message evaluation set.')
    path=ARTIFACTS/'stress_summary.json'
    if not path.exists(): st.info('Run the stress command to populate this experiment.')
    else:
